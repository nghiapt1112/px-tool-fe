    ...
    <vs-sidebar v-hammer:swipe.left="onSwipeLeft" ref="mainSidebar" :parent="parent" :hiddenBackground="clickNotClose"
      ...
      <div @mouseenter="sidebarMouseEntered" @mouseleave="sidebarMouseLeave">
          ...
          <div>
            ...
            <template v-else-if="!showCloseButton && !sidebarItemsMin">
              <feather-icon icon="DiscIcon" class="mr-0 cursor-pointer" svg-classes="stroke-current" v-show="!reduce"
                            @click="toggleReduce(true)" id="btnSidebarToggler"></feather-icon>
        ...
        <VuePerfectScrollbar ref="mainSidebarPs" class="scroll-area--main-sidebar pt-2" :settings="settings"
          ...
          <template v-for="(sidebarItem, index) in sidebarItemsRoleFilter">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      sidebarItemsRoleFilter: {
        get () {
          return this.sidebarItems.filter(item => {
            return !item.roles || item.roles.includes(this.AppActiveUser.type);
          })
        }
      },
      isSidebarActive: {
